...
export default function CommunityMembersPage() {
  ...
  return (
    <div className="p-8">
      ...
      <p className="mt-1 text-[#64748B]">View and manage your community members</p>
      ...
      {loading ? (
        ...
        <div className="mt-8 overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#E2E8F0] text-left text-sm text-[#64748B]">
            ...
            <tbody>
              {members.map((m) => (
                <tr key={m._id} className="border-b border-zinc-800 text-sm">
                  <td className="py-3 pr-4 text-[#0F172A]">{m.name}</td>
                  <td className="py-3 pr-4 text-[#64748B]">{m.email}</td>
